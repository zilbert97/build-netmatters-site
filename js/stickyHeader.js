...
function createScrollSpy(target) {
  ...
  const callback = function() {
    ...
    // If scrolled past the height of the target element
    else if (scrollTop > (targetHeight + 25)) {
      ...
      // On scroll up, if changed direction of scroll
        ...
        // Bug fix - header would not debounce properly
        target.css('opacity', '0');

        // Separate timeout used as clearTimeout() would not restore visibility
        setTimeout(function() {
          target.css('opacity', '1');
        }, 250);
...
function createScrollSpy(target) {
    ...
    // If scrolled past the height of the target element
      ...
      timeout = setTimeout(function() {
        ...
        // On scroll up, if changed direction of scroll
          ...
          // Show the header
          target.removeClass('slide-up').addClass('slide-down');
  ...
  // When page loaded if scrolled past header, stick header
  if ($(window).scrollTop() > parseInt(target.css('height'))) {
    makeSticky(target);
  }
